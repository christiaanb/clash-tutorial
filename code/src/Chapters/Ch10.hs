-- | The reader's @src/Example/Project.hs@ at the end of chapter 10.
--
-- Every definition is chapter 9's, unchanged. What chapter 10 adds is a test
-- bench: two boards written down from what the REPL printed, a @testBench@
-- built from @stimuliGenerator@ and @outputVerifier'@, and a @TestBench@
-- annotation tying it to @life@ so that one @:vhdl@ generates both.
--
-- @{-\# OPAQUE life \#-}@ is what makes the test bench instantiate the entity
-- rather than a second copy of its logic. A @Synthesize@ annotation names the
-- top of a design; it does not stop that design being inlined into something
-- else, and the test bench is something else.
--
-- @Clash.Explicit.Testbench@ is the second of the two imports the spine adds to
-- the single one chapter 1 starts with, and like the first it is in code that
-- prints rather than in the circuit.
module Chapters.Ch10 where
